...
export default function Uploads() {
  ...
  const handleFileUpload = async (fileList: File[]) => {
    ...
    for (const file of fileList) {
      if (!['application/pdf', 'text/plain', 'text/markdown'].includes(file.type)) {
        alert(`File type ${file.type} is not supported. Please upload PDF, TXT, or MD files.`);
        continue;
      }
  ...
  const getFileIcon = (type: string) => {
    ...
    if (type === 'text/markdown') return <File className="h-8 w-8 text-blue-500" />;
  ...
  const categoryStats = categories.map(category => ({
    name: category,
    count: files.filter(file => file.category === category && (isAdmin || file.uploadedBy === user?.name)).length
  }));
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="text-right">
          ...
          <p className="text-2xl font-bold text-primary">{filteredFiles.length}</p>
        </div>
      </div>
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {isAdmin ? 'Knowledge base organization' : 'Your document categories'}
              </CardDescription>
            </CardHeader>
            <CardContent>
          ...
          {isAdmin && (
            <Card>
              ...
              <CardContent className="space-y-3 text-sm text-gray-600">
                <p>• Organize documents by category to improve AI analysis accuracy</p>
                <p>• Uploaded documents are automatically processed and indexed</p>
                <p>• Users can only see their own uploads plus shared knowledge base</p>
